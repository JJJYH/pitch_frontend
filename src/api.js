...
const get = {
    ...
    userList: () => { return instance.get('/admin/list')},
}

const post = {
    //post 작성방법
    //api: (data) => { return instance.post('/api경로', data) }
    test: () => { return instance.post('/admin/test') },
}
...
const sort = {
  // 지원자 목록 api
  applicantList: (posting_no, type) => {return instance.get(`/admin/${posting_no}/sort?type=${type}`)}
